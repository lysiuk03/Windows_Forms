            ...
            else if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                foreach (var item in (string[])e.Data.GetData(DataFormats.FileDrop))
                {
                    if (Path.GetExtension(item)==".rtf")
                    {
                        ...
                        GetSelectedTb().LoadFile(item);
                        tabControl1.SelectedTab.Text = Path.GetFileName(item);
                    }
                    else
                    { 
                        MessageBox.Show("File extension must be  rich text!", "Open!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
        }
